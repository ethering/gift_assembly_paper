...
use warnings;
use FindBin;
use lib "$FindBin::Bin/FAlite";
#use lib '/usr/users/TGAC_ga004/ethering/perl_scripts/fasta_scripts/FAlite';
use FAlite;
use Getopt::Long;
...
my $cutoff = 0; 	# the minimum sequence length to consider in stats
...
options:
	...
	-cutoff <int> the minimum sequence length to consider in stats
";
...
sub sequence_statistics{
	...
	if ($type eq 'scaffold' && defined($genome_size)){		
		my $percent = sprintf("%.3f",($total_size / $genome_size) * 100);
	...
	foreach my $size (qw(1000 10000 100000 1000000 10000000 47000000 100000000)){
		my $matches = grep { $_ > $size } @{$data{$type}{lengths}};
		my $percent = sprintf("%.3f", ($matches / $count) * 100);
